...
/*	开发中常用的字符串函数 [看手册或者官方编程指南]
1） 统计宇符串的长度，按字节： len(str)
2） 字符串遍历，同时处理有中文的问题： r:=[]rune[str]
3） 字符串转整数： n,err := strconv.Atoi["12")
4） 整数转宇符串： str = strconv.ltoa(12345)
5） 字符串转[]byte： varbytes = []]byte("hello go")
6） []byte转字符串：str = string([]byte{97,98,99})
7） 10进制转2,8,16进制：str=strconv.Formatlnt(123,2) //  12->8,16
8） 查找子串是否在指定的字符串中：strings.Contains("seafood”,"foo")  // true
9） 统计一个字符串有几个指定的子串：strings.Count("ceheese',"e")  // 4
10) 不区分大小写的字符串比较(==是区分字母大小写的)：fmt.Printin(strings.EqualFold("abc","Abo")  // true
11) 返回子串在字符串第一次出现的index值，如果没有返回-1： strings.lndex("NLT_abc", "abc") // 4
12) 返回子串在字符串最后一次出现的index，如没有返回-1：strings.Lastindex("go golang”,"go"）
13) 将指定的子串替换成另外一个子串：strings.Replace(" go go hello"，"go"，"Bo语言" ,n) //n可以指定你希望替换几个，如果n=-1表示全部替換
14) 按照指定的某个字符为分割标识，将一个字符串拆分成宇符串数组： strings.Split("hello,wrold,ok", ",")
...
19) 将字符串右边指定的宇符去掉：strings.TrimRight("! hello!"," !")   // 将右边的！和""去掉
